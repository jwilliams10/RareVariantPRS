...
for(trait in c("BMI","TC","HDL","LDL","logTG","Height")){
  ...
  pvals <- unique(read.csv(paste0("/data/williamsjacr/UKB_WES_Phenotypes/Continuous/Results/GeneCentricNoncoding/",trait,"_noncoding_sig.csv")))
...
for(trait in c("BMI","TC","HDL","LDL","logTG","Height")){
  ...
  pvals <- unique(read.csv(paste0("/data/williamsjacr/UKB_WES_Phenotypes/Continuous/Results/GeneCentricCoding/",trait,"_coding_sig.csv")))
