...
# 
# Final_Coefficients_SL <- read.csv(paste0("/data/williamsjacr/UKB_WES_Phenotypes/Binary/Results/Combined_Common_PRS/",trait,"_final_coef.csv"))
# 
# CV_SL_Beta <- Final_Coefficients_CT[,c("CHR","SNP","REF","BP","A1")]
# CV_SL_Beta$BETA <- 0
# 
# for(i in 1:nrow(Final_Coefficients_SL)){
#   name_i <- Final_Coefficients_SL$Coef[i]
#   if(name_i %in% colnames(All)){
#     CV_SL_Beta$BETA <- CV_SL_Beta$BETA + All[,name_i]*Final_Coefficients_SL$Beta[i]
#   }
# }
# 
...
# all.equal(Best_Validation_All$prs,exp(Final_Coefficients_SL$Beta[1] + test_validation[,5])/(1 + exp(Final_Coefficients_SL$Beta[1] + test_validation[,5])))
# cor(Best_Validation_All$prs,exp(Final_Coefficients_SL$Beta[1] + test_validation[,5])/(1 + exp(Final_Coefficients_SL$Beta[1] + test_validation[,5])))
