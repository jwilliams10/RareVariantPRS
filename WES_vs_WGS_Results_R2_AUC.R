...
for(trait in c("BMI","TC","HDL","LDL","logTG","Height")){
  ...
  RICE_CV_Results <- read.csv(paste0("/data/williamsjacr/UKB_WES_Phenotypes/Continuous/Results/Combined_Common_PRS/",trait,"Best_Betas.csv"))
  RICE_CV_Results$Method <- "RICE-CV"
  RICE_RV_Results <- read.csv(paste0("/data/williamsjacr/UKB_WES_Phenotypes/Continuous/Results/Combined_RareVariants_PRS/",trait,"Best_Betas.csv"))
...
for(trait in c("Asthma","Breast","CAD","Prostate","T2D")){
  ...
  LASSOSUM2_Results$Method <- "Lassosum2"
  RICE_CV_Results <- read.csv(paste0("/data/williamsjacr/UKB_WES_Phenotypes/Binary/Results/Combined_Common_PRS/",trait,"Best_Betas.csv"))
  RICE_CV_Results$Method <- "RICE-CV"
  RICE_RV_Results <- read.csv(paste0("/data/williamsjacr/UKB_WES_Phenotypes/Binary/Results/Combined_RareVariants_PRS/",trait,"Best_Betas.csv"))
  RICE_RV_Results$Method <- "RICE-RV"
...
for(trait in c("BMI","TC","HDL","LDL","logTG","Height")){
  ...
  LASSOSUM2_Results$Method <- "Lassosum2"
  RICE_CV_Results <- read.csv(paste0("/data/williamsjacr/UKB_WES_Phenotypes/Imputed/Results/SingleTrait_Ensemble/",trait,"Best_Betas.csv"))
  RICE_CV_Results$Method <- "RICE-CV"
  RICE_RV_Results <- read.csv(paste0("/data/williamsjacr/UKB_WES_Phenotypes/Imputed/Results/SingleTrait_Ensemble_RV/",trait,"Best_Betas.csv"))
  RICE_RV_Results$Method <- "RICE-RV"
...
for(trait in c("Asthma","Breast","CAD","Prostate","T2D")){
  ...
  LASSOSUM2_Results$Method <- "Lassosum2"
  RICE_CV_Results <- read.csv(paste0("/data/williamsjacr/UKB_WGS_Results_Binary/Combined_Common_PRS/",trait,"Best_Betas.csv"))
  RICE_CV_Results$Method <- "RICE-CV"
  RICE_RV_Results <- read.csv(paste0("/data/williamsjacr/UKB_WGS_Results_Binary/BestRareVariantPRS/",trait,"_Best_Betas.csv"))
  RICE_RV_Results$Method <- "RICE-RV"
...
for(trait in c("BMI","TC","HDL","LDL","logTG","Height")){
  ...
  LASSOSUM2_Results$Method <- "Lassosum2"
  RICE_CV_Results <- read.csv(paste0("/data/williamsjacr/UKB_WGS_Results/Combined_Common_PRS/",trait,"Best_Betas.csv"))
  RICE_CV_Results$Method <- "RICE-CV"
  RICE_RV_Results <- read.csv(paste0("/data/williamsjacr/UKB_WGS_Results/BestRareVariantPRS/",trait,"_Best_Betas.csv"))
  RICE_RV_Results$Method <- "RICE-RV"
...
scale_fill_Publication <- function(...){
  library(scales)
  discrete_scale("fill","Publication",manual_pal(values = c("#DFB9DE","#C781C4","#973999","#F5B8B8","#EF7474","#E23838")), ...)
}
scale_colour_Publication <- function(...){
  library(scales)
  discrete_scale("colour","Publication",manual_pal(values = c("#DFB9DE","#C781C4","#973999","#F5B8B8","#EF7474","#E23838")), ...)
}
...
full_results_Continuous$R2_adjusted[full_results_Continuous$R2_adjusted < 0] <- 0
full_results_Binary$AUC_adjusted[full_results_Binary$AUC_adjusted < 0] <- 0
...
scale_fill_Publication_tmp <- function(...){
  library(scales)
  discrete_scale("fill","Publication",manual_pal(values = c("#DFB9DE","#F5B8B8","#C781C4","#EF7474","#973999","#E23838")), ...)
}

ylim_continuous <- max(full_results_Continuous$R2_adjusted) + 0.03
ylim_binary <- max(full_results_Binary$AUC_adjusted) + 0.03

plot1 <- ggplot(full_results_Continuous[full_results_Continuous$ancestry == "EUR",]) +
  geom_bar(aes(x=Method, y=abs(R2_adjusted),fill=Method_DataSource),position = "dodge", stat="identity", alpha=0.7) +
...
plot2 <- ggplot(full_results_Binary[full_results_Binary$ancestry == "EUR",]) +
  geom_bar(aes(x=Method, y=abs(AUC_adjusted),fill=Method_DataSource),position = "dodge", stat="identity", alpha=0.7) +
  facet_grid(cols = vars(trait)) +
  ylab("AUC of PRS") +
  coord_cartesian(ylim = c(0.4,ylim_binary)) +
  theme_Publication() +
  scale_fill_Publication() + guides(fill=guide_legend(title="PRS Method & Dataset"))
...
full_results_tmp <- full_results_Continuous
full_results_tmp <- full_results_tmp[full_results_tmp$Method %in% c("RICE-CV"),]
full_results_tmp$Method_DataSource <- gsub("RICE-CV & ","",full_results_tmp$Method_DataSource)
full_results_tmp$Method_DataSource <- factor(full_results_tmp$Method_DataSource,levels = c("WES","Imputed + WES","WGS"))

scale_fill_Publication_tmp <- function(...){
  library(scales)
  discrete_scale("fill","Publication",manual_pal(values = c("#DFB9DE","#C781C4","#973999")), ...)
}

plot_tmp1 <- ggplot(full_results_tmp[full_results_tmp$ancestry == "AFR",]) +
  geom_bar(aes(x=Method, y=abs(R2_adjusted),fill=Method_DataSource),position = "dodge", stat="identity", alpha=0.7) +
  facet_grid(cols = vars(trait)) +
  ylab("R2 of PRS") +
  ylim(0,ylim_continuous) +
  theme_Publication() +
  scale_fill_Publication_tmp() + guides(fill=guide_legend(title="RICE-CV"))

legend_tmp1 <- ggplotGrob(plot_tmp1)$grobs[[which(sapply(ggplotGrob(plot_tmp1)$grobs, function(x) x$name) == "guide-box")]]

full_results_tmp <- full_results_Continuous
full_results_tmp <- full_results_tmp[full_results_tmp$Method %in% c("RICE-RV"),]
full_results_tmp$Method_DataSource <- gsub("RICE-RV & ","",full_results_tmp$Method_DataSource)
full_results_tmp$Method_DataSource <- factor(full_results_tmp$Method_DataSource,levels = c("WES","Imputed + WES","WGS"))

scale_fill_Publication_tmp <- function(...){
  library(scales)
  discrete_scale("fill","Publication",manual_pal(values = c("#F5B8B8","#EF7474","#E23838")), ...)
}

plot_tmp2 <- ggplot(full_results_tmp[full_results_tmp$ancestry == "AFR",]) +
  geom_bar(aes(x=Method, y=abs(R2_adjusted),fill=Method_DataSource),position = "dodge", stat="identity", alpha=0.7) +
  facet_grid(cols = vars(trait)) +
  ylab("R2 of PRS") +
  ylim(0,ylim_continuous) +
  theme_Publication() +
  scale_fill_Publication_tmp() + guides(fill=guide_legend(title="RICE-RV"))

legend_tmp2 <- ggplotGrob(plot_tmp2)$grobs[[which(sapply(ggplotGrob(plot_tmp2)$grobs, function(x) x$name) == "guide-box")]]
...
legend_b <- plot_grid(legend_tmp1,NULL,legend_tmp2,align="h",nrow = 1,rel_widths = c(1,-0.4,1))
