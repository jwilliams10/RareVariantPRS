    ...
    for(k in c("Causal Prop. 0.2","Causal Prop. 0.05","Causal Prop. 0.01")){
      for(q in c("n = 49,173","n = 98,343")){
        for(ancs in c("AFR","AMR","EUR","SAS")){
          ...
          ratio_RV_Add_Beta <- (data$Beta[data$Method == "RICE-CV"] + data$Beta[data$Method == "RICE-RV"])/data$Beta[data$Method == "RICE-CV"] - 1
          ratio_RV_Add_R2 <- (data$R2[data$Method == "RICE-CV"] + data$R2[data$Method == "RICE-RV"])/data$R2[data$Method == "RICE-CV"] - 1
          ratio_CV_Best_Beta <- data$Beta[data$Method == "RICE-CV"]/max(data$Beta[!(data$Method %in% c("RICE-CV","RICE-RV"))]) - 1
          ratio_CV_Best_R2 <- data$R2[data$Method == "RICE-CV"]/max(data$R2[!(data$Method %in% c("RICE-CV","RICE-RV"))]) - 1
          
          beta_ratios <- rbind(beta_ratios,data.frame(Causality_Structure = caus_struct,Scale = i, Causal_Prop = k, Training_Size = q, Ancestry = ancs,ratio_RV_Add_Beta = ratio_RV_Add_Beta,ratio_RV_Add_R2 =ratio_RV_Add_R2,ratio_CV_Best_Beta = ratio_CV_Best_Beta,ratio_CV_Best_R2 = ratio_CV_Best_R2))
...
mean(beta_ratios$ratio_CV_Best_Beta[beta_ratios$Causal_Prop %in% c("Causal Prop. 0.2","Causal Prop. 0.05","Causal Prop. 0.01")])

mean(beta_ratios$ratio_CV_Best_R2[beta_ratios$Causal_Prop %in% c("Causal Prop. 0.2","Causal Prop. 0.05","Causal Prop. 0.01")])

mean(beta_ratios$ratio_RV_Add_Beta[beta_ratios$Ancestry == "AFR" & beta_ratios$Causal_Prop %in% c("Causal Prop. 0.2","Causal Prop. 0.05","Causal Prop. 0.01")])
mean(beta_ratios$ratio_RV_Add_Beta[beta_ratios$Ancestry == "EUR" & beta_ratios$Causal_Prop %in% c("Causal Prop. 0.2","Causal Prop. 0.05","Causal Prop. 0.01")])
mean(beta_ratios$ratio_RV_Add_Beta[beta_ratios$Ancestry == "SAS" & beta_ratios$Causal_Prop %in% c("Causal Prop. 0.2","Causal Prop. 0.05","Causal Prop. 0.01")])
mean(beta_ratios$ratio_RV_Add_Beta[beta_ratios$Ancestry == "AMR" & beta_ratios$Causal_Prop %in% c("Causal Prop. 0.2","Causal Prop. 0.05","Causal Prop. 0.01")])
