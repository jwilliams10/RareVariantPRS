# =============================================================================
# [RICE-ANNOTATION] DNANexus/Single_RareVariant_PRS_All.R
...
RV_PRS <- inner_join(pheno_valid[,c("IID","y_validation","age","age2","sex","pc1","pc2","pc3","pc4","pc5","pc6","pc7","pc8","pc9","pc10")],RV_Validation_Coding_PRS)
...
Beta_Boot <- function(data,indices){
  ...
  result <- coef(lm(as.formula(paste0("y_validation~","RV_Coding_PRS")),data = boot_data))[2]
  return(c(result))
}

R2_Boot <- function(data,indices){
  boot_data <- data[indices, ]
  result <- summary(lm(as.formula(paste0("y_validation~","RV_Coding_PRS")),data = boot_data))$r.squared
  return(c(result))
}
...
RV_PRS <- inner_join(pheno_valid[,c("IID","y_validation","age","age2","sex","pc1","pc2","pc3","pc4","pc5","pc6","pc7","pc8","pc9","pc10")],RV_Validation_Noncoding_PRS)
...
Beta_Boot <- function(data,indices){
  boot_data <- data[indices, ]
  result <- coef(lm(as.formula(paste0("y_validation~","RV_Noncoding_PRS")),data = boot_data))[2]
  return(c(result))
}

R2_Boot <- function(data,indices){
  boot_data <- data[indices, ]
  result <- summary(lm(as.formula(paste0("y_validation~","RV_Noncoding_PRS")),data = boot_data))$r.squared
  return(c(result))
}
...
mod <- lm(as.formula(paste0("y_tune~RV_Coding_PRS+RV_Noncoding_PRS")),data = pheno_tune)

pheno_tune$RV_PRS <- predict(mod,pheno_tune)
pheno_valid$RV_PRS <- predict(mod,pheno_valid)
...
Beta_Boot <- function(data,indices){
  boot_data <- data[indices, ]
  result <- coef(lm(as.formula(paste0("y_validation~","RV_PRS")),data = boot_data))[2]
  return(c(result))
}

R2_Boot <- function(data,indices){
  boot_data <- data[indices, ]
  result <- summary(lm(as.formula(paste0("y_validation~","RV_PRS")),data = boot_data))$r.squared
  return(c(result))
}
