...
X_train <- data.frame(IID = ids_gstar[ids_gstar %in% obj_nullmodel_train$id_include],G_star_gene_centric_coding_train,G_star_gene_centric_noncoding_train)
...
X_tune <- data.frame(IID = ids_gstar[ids_gstar %in% obj_nullmodel_tune$id_include],G_star_gene_centric_coding_tune,G_star_gene_centric_noncoding_tune)
...
beta_matrix <- cbind(rbind(Coding_Train_PVals_All[,c(1:4)],Noncoding_Train_PVals_All[,c(1:4)]),beta_matrix)
...
if(r2_sl_tune < r2_bestoverall_tune){
  ...
  write.csv(beta_matrix[,c("Gene","Chr","Category","Number_SNV","Beta_Final")],file = paste0(trait,"_Modified_Sole_Coefficients.csv"),row.names = FALSE)
}else{
  if(best_algorithm == "SL.glmnet_All"){
...
write.csv(RV_PRS,file = paste0(trait,"_Modified_BestPRS.csv"),row.names = FALSE)
...
write.csv(RV_PRS_Results,file = paste0(trait,"_Modified_Best_Betas.csv"),row.names = FALSE)
