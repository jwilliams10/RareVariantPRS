...
beta_validation_raw_AMR <- coef(glm(as.formula(paste0(trait,"~p_value_",idx,"+",gsub("~","",confounders))),data = pheno_validation_raw_AMR,family = binomial()))[2]
se_validation_raw_AMR <- summary(glm(as.formula(paste0(trait,"~p_value_",idx,"+",gsub("~","",confounders))),data = pheno_validation_raw_AMR,family = binomial()))$coefficients[2,2]
...
beta_validation_adjusted_AMR <- coef(glm(as.formula(paste0(trait,"~p_value_",idx,"+",gsub("~","",confounders))),data = pheno_validation_adjusted_AMR,family = binomial()))[2]
se_validation_adjusted_AMR <- summary(glm(as.formula(paste0(trait,"~p_value_",idx,"+",gsub("~","",confounders))),data = pheno_validation_adjusted_AMR,family = binomial()))$coefficients[2,2]
