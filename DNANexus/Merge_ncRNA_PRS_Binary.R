...
# dx run app-swiss-army-knife -iin=UKB_PRS:JW/Software/r_with_plink.tar.gz -iin=UKB_PRS:JW/UKB_Phenotypes/Scripts/Binary/RareVariant_PRS/Merge_ncRNA_PRS_Binary.R -iin=UKB_PRS:JW/UKB_Phenotypes/Scripts/Binary/RareVariant_PRS/Merge_ncRNA_PRS_Binary.sh -icmd="bash Merge_ncRNA_PRS_Binary.sh" -y --destination UKB_PRS:JW/UKB_Phenotypes/Results/Binary/GeneCentric_ncRNA/ --priority high --extra-args '{"executionPolicy":{"maxSpotTries":5,"spotOnly":true}}' --instance-type mem3_ssd1_v2_x8
...
for(trait in c("Asthma","T2D","Breast","Prostate","CAD")){
  PRS <- read.csv(paste0("GeneCentric_ncRNA_PRS/",trait,"_ncRNA_PRS1172.csv")) 
  IDs <- PRS$ID
  ...
  for(array in 1:1172){
    ...
    tmp <- read.csv(paste0("GeneCentric_ncRNA_PRS/",trait,"_ncRNA_PRS",array,".csv"))
  ...
  write.csv(PRS_Final_Burden,file = paste0(trait,"_ncRNA_Burden_PRS.csv"),row.names = FALSE)
  write.csv(PRS_Final_STAARO,file = paste0(trait,"_ncRNA_STAARO_PRS.csv"),row.names = FALSE)
}

system("rm -r GeneCentric_ncRNA_PRS/")
