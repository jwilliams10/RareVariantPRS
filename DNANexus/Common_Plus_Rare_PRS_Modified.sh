...
dx download UKB_PRS:JW/UKB_Phenotypes/Results/Continuous/BestRareVariantPRS/${trait}_Modified_BestPRS.csv
dx download UKB_PRS:JW/UKB_Phenotypes/Results/all_phenotypes.RData

# mount PWD and run bigsnpr using my_r_script.r
docker run -v $PWD:/data -w /data --entrypoint /bin/bash r_with_plink -l -c "Rscript Common_Plus_Rare_PRS_Modified.R ${1}"