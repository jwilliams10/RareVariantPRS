...
CHR=$(sed "${2}q;d" noncoding_chr.csv | awk -F ',' '{print $2}')

dx download UKB_PRS:JW/UKB_Phenotypes/Results/Binary/GeneCentricNoncoding/${trait}_noncoding_sig_es.csv
...
docker run -v $PWD:/data -w /data --entrypoint /bin/bash r_with_plink -l -c "Rscript GeneCentric_Noncoding_PRS_Binary.R ${2}"