...
ggsave(paste0("Desktop/RareVariantPRS_Results/Figures/UKB_WGS_Continuous_","A","_Raw_vs_AncestryAdjusted_Beta.png"),g1,width=10, height=6.18047,dpi = 300)
...
g3 <- ggplot(CV_Coding_Noncoding) +
  ...
  facet_grid(vars(trait), vars(ancestry)) + 
  ggtitle("UKB WGS PRS Coding vs Noncoding for Six Continuous Traits") + 
  ylab("Beta of PRS per SD") + 
  ylim(0,0.7) +
  theme_Publication() + 
  scale_fill_Publication()
...
g4 <- ggplot(CV_Coding_Noncoding) +
  ...
  facet_grid(vars(trait), vars(ancestry)) + 
  ggtitle("UKB WGS Ancestry Adjusted PRS Coding vs Noncoding for Six Continuous Traits") + 
  ylab("Beta of PRS per SD") + 
  ylim(0,0.7) +
  theme_Publication() + 
  scale_fill_Publication()
