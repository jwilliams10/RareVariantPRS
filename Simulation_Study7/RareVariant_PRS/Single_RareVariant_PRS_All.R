...
Train_PVals_All <- get(load(paste0("/data/williamsjacr/UKB_WES_Simulation/Simulation7/Results/GeneCentricCoding/coding_sig",i,".Rdata")))
Train_PVals_All <- Train_PVals_All[Train_PVals_All$STAARB <= 1e-03,]
system(paste0("rm ",paste0("/data/williamsjacr/UKB_WES_Simulation/Simulation7/Results/GeneCentricCoding/coding_sig",i,".Rdata")))

### PATCH: flag for zero-PRS case
zero_prs <- FALSE
...
## Null Model
obj_nullmodel_train <- get(load(paste0("/data/williamsjacr/UKB_WES_Simulation/Simulation7/nullmodels_staar/Train_Null_Model",i,".RData")))
obj_nullmodel_tune <- get(load(paste0("/data/williamsjacr/UKB_WES_Simulation/Simulation7/nullmodels_staar/Tune_Null_Model",i,".RData")))
obj_nullmodel_validation <- get(load(paste0("/data/williamsjacr/UKB_WES_Simulation/Simulation7/nullmodels_staar/Validation_Null_Model",i,".RData")))

system(paste0("rm ",paste0("/data/williamsjacr/UKB_WES_Simulation/Simulation7/nullmodels_staar/Train_Null_Model",i,".RData")))
system(paste0("rm ",paste0("/data/williamsjacr/UKB_WES_Simulation/Simulation7/nullmodels_staar/Tune_Null_Model",i,".RData")))
system(paste0("rm ",paste0("/data/williamsjacr/UKB_WES_Simulation/Simulation7/nullmodels_staar/Validation_Null_Model",i,".RData")))
...
### PATCH: wrap G* computation in tryCatch; mark zero_prs if Train_PVals_All is empty
tryCatch({
  
  if (nrow(Train_PVals_All) == 0) {
    stop("Train_PVals_All_empty")
  }
  ...
  if (grepl("Train_PVals_All_empty", conditionMessage(e))) {
    zero_prs <<- TRUE
    message("Train_PVals_All is empty; will set PRS_Tune and PRS_Validation to zero vectors.")
  } else {
    stop(e)
  }
})
...
if (!zero_prs) {
  pheno_train <- inner_join(pheno_train, X_train, by = "IID")
  
  if (ncol(pheno_train) <= 2) {
    ## No G* columns ended up in the data (only IID and Y)
    message("No G* predictors after filtering/join; setting zero_prs = TRUE")
    zero_prs <- TRUE
    X_train <- NULL
  } else {
  ...
  X_train <- NULL  # not used in zero-PRS case
}
...
if (!zero_prs) {
  pheno_tune <- inner_join(pheno_tune, X_tune, by = "IID")
...
if (!zero_prs) {
  pheno_validation <- inner_join(pheno_validation, X_valid, by = "IID")
...
if (!zero_prs) {
  ...
  if (p == 1) {
    ## ---- SINGLE-PREDICTOR CASE: NO GLMNET ----
    ## simple linear model on the single G* column
    ...
    ## make "lasso" and "ridge" PRS identical to lm PRS
    lasso_prs_tune <- matrix(lm_prs_tune, ncol = 1)
    ridge_prs_tune <- matrix(lm_prs_tune, ncol = 1)
    lasso_prs_vad  <- matrix(lm_prs_vad,  ncol = 1)
    ridge_prs_vad  <- matrix(lm_prs_vad,  ncol = 1)
    
    lasso_tune_dat  <- data.frame(y = pheno_tune$y_tune,      lasso_prs_tune)
    colnames(lasso_tune_dat) <- c("y", "lasso_prs1")
    lasso_valid_dat <- data.frame(y = pheno_validation$y_validation, lasso_prs_vad)
    colnames(lasso_valid_dat) <- c("y", "lasso_prs1")
    
    ridge_tune_dat  <- data.frame(y = pheno_tune$y_tune,      ridge_prs_tune)
    colnames(ridge_tune_dat) <- c("y", "ridge_prs1")
    ridge_valid_dat <- data.frame(y = pheno_validation$y_validation, ridge_prs_vad)
    colnames(ridge_valid_dat) <- c("y", "ridge_prs1")
    
    lm_tune_dat  <- data.frame(y = pheno_tune$y_tune,      lm_prs_tune)
    colnames(lm_tune_dat) <- c("y", "lm_prs")
    lm_valid_dat <- data.frame(y = pheno_validation$y_validation, lm_prs_vad)
    colnames(lm_valid_dat) <- c("y", "lm_prs")
    
    all_prs_tune  <- cbind(lasso_prs_tune, ridge_prs_tune, lm_prs_tune)
    colnames(all_prs_tune) <- c("lasso_prs1", "ridge_prs1", "lm_prs")
    all_prs_valid <- cbind(lasso_prs_vad,  ridge_prs_vad,  lm_prs_vad)
    colnames(all_prs_valid) <- c("lasso_prs1", "ridge_prs1", "lm_prs")
    
  } else if (p >= 2) {
    ## ---- ORIGINAL MULTI-PREDICTOR CASE ----
    ...
    all_prs_tune  <- cbind(lasso_prs_tune, ridge_prs_tune, lm_prs_tune)
    colnames(all_prs_tune) <- c(paste0("lasso_prs", 1:ncol(lasso_prs_tune)),
                                paste0("ridge_prs", 1:ncol(ridge_prs_tune)),
                                "lm_prs")
    all_prs_valid <- cbind(lasso_prs_vad, ridge_prs_vad, lm_prs_vad)
    colnames(all_prs_valid) <- c(paste0("lasso_prs", 1:ncol(lasso_prs_vad)),
                                 paste0("ridge_prs", 1:ncol(ridge_prs_vad)),
                                 "lm_prs")
  }
  ...
  ### PATCH: zero-PRS case
  message("zero_prs = TRUE → setting PRS_Tune and PRS_Validation to 0")
  PRS_Tune <- rep(0, nrow(pheno_tune))
  PRS_Validation <- rep(0, nrow(pheno_validation))
}
...
write.csv(PRS_Tune,paste0("/data/williamsjacr/UKB_WES_Simulation/Simulation7/Results/Combined_RareVariants_PRS/PRS_Tune_",i,".csv"),row.names = FALSE)
PRS_Validation <- data.frame(IID = pheno_validation$IID,PRS = PRS_Validation)
write.csv(PRS_Validation,paste0("/data/williamsjacr/UKB_WES_Simulation/Simulation7/Results/Combined_RareVariants_PRS/PRS_Validation_",i,".csv"),row.names = FALSE)
...
Beta_Boot <- function(data,indices){
  ...
  result <- tryCatch(coef(lm(as.formula(paste0("y_validation~","PRS")),data = boot_data))[2],error = function(e){return(0)})
  return(c(result))
}

R2_Boot <- function(data,indices){
  boot_data <- data[indices, ]
  result <- tryCatch(summary(lm(as.formula(paste0("y_validation~","PRS")),data = boot_data))$r.squared,error = function(e){return(0)})
  return(c(result))
}

beta_validation_raw_EUR <- tryCatch(coef(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_raw_EUR))[2],error = function(e){return(0)})
...
R2_validation_raw_EUR <- tryCatch(summary(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_raw_EUR))$r.squared,error = function(e){return(0)})
...
beta_validation_raw_SAS <- tryCatch(coef(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_raw_SAS))[2],error = function(e){return(0)})
...
R2_validation_raw_SAS <- tryCatch(summary(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_raw_SAS))$r.squared,error = function(e){return(0)})
...
beta_validation_raw_AMR <- tryCatch(coef(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_raw_AMR))[2],error = function(e){return(0)})
...
R2_validation_raw_AMR <- tryCatch(summary(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_raw_AMR))$r.squared,error = function(e){return(0)})
...
beta_validation_raw_AFR <- tryCatch(coef(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_raw_AFR))[2],error = function(e){return(0)})
...
R2_validation_raw_AFR <- tryCatch(summary(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_raw_AFR))$r.squared,error = function(e){return(0)})
...
beta_validation_adjusted_EUR <- tryCatch(coef(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_adjusted_EUR))[2],error = function(e){return(0)})
...
R2_validation_adjusted_EUR <- tryCatch(summary(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_adjusted_EUR))$r.squared,error = function(e){return(0)})
...
beta_validation_adjusted_SAS <- tryCatch(coef(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_adjusted_SAS))[2],error = function(e){return(0)})
...
R2_validation_adjusted_SAS <- tryCatch(summary(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_adjusted_SAS))$r.squared,error = function(e){return(0)})
...
beta_validation_adjusted_AMR <- tryCatch(coef(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_adjusted_AMR))[2],error = function(e){return(0)})
...
R2_validation_adjusted_AMR <- tryCatch(summary(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_adjusted_AMR))$r.squared,error = function(e){return(0)})
...
beta_validation_adjusted_AFR <- tryCatch(coef(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_adjusted_AFR))[2],error = function(e){return(0)})
...
R2_validation_adjusted_AFR <- tryCatch(summary(lm(as.formula(paste0("y_validation~","PRS")),data = RV_PRS_adjusted_AFR))$r.squared,error = function(e){return(0)})
...
write.csv(RICE_RV,file = paste0("/data/williamsjacr/UKB_WES_Simulation/Simulation7/Results/Combined_RareVariants_PRS/Best_Betas",i,".csv"),row.names = FALSE)
