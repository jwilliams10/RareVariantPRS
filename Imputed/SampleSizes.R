# =============================================================================
# [RICE-ANNOTATION] Imputed/SampleSizes.R
...
samps <- read.delim("/data/williamsjacr/UKB_WES_Phenotypes/Imputed/BEDFiles/ukb_hm3_mega.fam",sep = " ",header = FALSE)

pheno_train <- read.delim("/data/williamsjacr/UKB_WES_Phenotypes/All_Train.txt")
pheno_train <- pheno_train[pheno_train$IID %in% samps[,2],]
pheno_tuning <- read.delim("/data/williamsjacr/UKB_WES_Phenotypes/All_Tune.txt")
pheno_tuning <- pheno_tuning[pheno_tuning$IID %in% samps[,2],]
pheno_vad <- read.delim("/data/williamsjacr/UKB_WES_Phenotypes/All_Validation.txt")
pheno_vad <- pheno_vad[pheno_vad$IID %in% samps[,2],]
