# =============================================================================
# [RICE-ANNOTATION] Imputed/RareVariant_PRS/Single_RareVariant_PRS_All.R
# Purpose: Constructs the rare-variant PRS component (RICE-RV) from burden scores and selected rare-variant sets.
#
# Paper linkage:
...
time <- system.time({
  ...
  # col_remove <- apply(G_star_gene_centric_coding,2,function(x){sum(x[(x - as.integer(x))==0] != 0)}) > 10 & colSums(G_star_gene_centric_coding) > 10
  # sum(!col_remove)
  # which(!col_remove)
  # 
  # col_remove <- apply(G_star_gene_centric_coding,2,function(x){sum(x > 0.1)}) > 10 & colSums(G_star_gene_centric_coding) > 10
  # sum(!col_remove)
  # which(!col_remove)
  # 
  # col_remove <- apply(G_star_gene_centric_coding,2,function(x){sum(x >= 1)}) > 15 & colSums(G_star_gene_centric_coding) > 10
  # sum(!col_remove)
  # which(!col_remove)
  ...
  G_star_gene_centric_coding_train <- G_star_gene_centric_coding[ids_gstar %in% pheno_train$IID,]
  G_star_gene_centric_coding_tune <- G_star_gene_centric_coding[ids_gstar %in% pheno_tune$IID,]
  G_star_gene_centric_coding_vad <- G_star_gene_centric_coding[ids_gstar %in% pheno_valid$IID,]
  ...
  if(trait %in% binary_traits){
    ...
    write.csv(Final_Coefficients,file = paste0("/data/williamsjacr/UKB_WES_Phenotypes/Imputed/Results/SingleTrait_Ensemble_RV/",trait,"_Coefficients.csv"),row.names = FALSE)
    ...
    write.csv(PRS_Tune,paste0("/data/williamsjacr/UKB_WES_Phenotypes/Imputed/Results/SingleTrait_Ensemble_RV/",trait,"_PRS_Tune.csv"),row.names = FALSE)
    PRS_Validation <- data.frame(IID = pheno_validation$IID,PRS = PRS_Validation)
    write.csv(PRS_Validation,paste0("/data/williamsjacr/UKB_WES_Phenotypes/Imputed/Results/SingleTrait_Ensemble_RV/",trait,"_PRS_Validation.csv"),row.names = FALSE)
...
save(time,file = paste0("/data/williamsjacr/UKB_WES_Phenotypes/Imputed/Results/SingleTrait_Ensemble_RV/",trait,"_Time.RData"))