# =============================================================================
# [RICE-ANNOTATION] Imputed/RareVariant_PRS/RICE_RV_Lipids_Gene.R
...
Gene_Centric_Coding_G_Star <- function(chr,gene_name,
...
continuous_traits <- c("TC","HDL","LDL","logTG")
...
Train_PVals_All <- Train_PVals_All[Train_PVals_All$Gene %in% c("LDLR","APOB","PCSK9"),]
Train_PVals_All <- unique(Train_PVals_All[,c("Gene","Chr")])
...
for(i in 1:nrow(Train_PVals_All)){
  ...
  G_star_gene_centric_coding[[i]] <- Gene_Centric_Coding_G_Star(chr=chr,gene_name=gene_name,
                                                                genofile,obj_nullmodel,rare_maf_cutoff=0.01,rv_num_cutoff=2,
...
if(trait %in% binary_traits){
  ...
  if(ncol(all_prs_tune) == 1){
    Results <- list()
    Results$Coefficients <- c(0,1)
    names(Results$Coefficients) <- c("Intercept",colnames(all_prs_tune))
  }else{
...
write.csv(RV_PRS_Results,file = paste0("/data/williamsjacr/UKB_WES_Phenotypes/Imputed/Results/SingleTrait_Ensemble_RV/",trait,"_Lipids_Gene_Best_Betas.csv"),row.names = FALSE)
write.csv(RV_Boot_Results,file = paste0("/data/williamsjacr/UKB_WES_Phenotypes/Imputed/Results/SingleTrait_Ensemble_RV/",trait,"_Lipids_Gene_Bootstraps.csv"),row.names = FALSE)
