# =============================================================================
# [RICE-ANNOTATION] Imputed/RareVariant_Analysis/STAAR_GeneCentric_Coding_LongMasks.R
# Purpose: Runs gene-centric rare-variant association testing using STAAR/STAARpipeline outputs.
#
# Paper linkage:
...
time <- system.time({
  ...
  arrayid_longmask <- index %% 2
  if(arrayid_longmask == 0){
    arrayid_longmask <- 2
    trait <- c(continuous_traits,binary_traits)[index/2]
  }else{
    trait <- c(continuous_traits,binary_traits)[floor(index/2) + 1]
  }
  ...
  ## Null Model
  obj_nullmodel <- get(load(paste0("/data/williamsjacr/UKB_WES_Phenotypes/Imputed/nullmodels_staar/",trait,"_Train_Null_Model.RData")))
  ...
  ## output path
  output_path <- "/data/williamsjacr/UKB_WES_Phenotypes/Imputed/Results/GeneCentricCoding/"
  ## output file name
...
save(time,file=paste0(output_path,output_file_name,"_",arrayid_longmask+379,"_Time.Rdata"))